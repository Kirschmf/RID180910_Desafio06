  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          title: "Erro",
          description: error.error || "Erro ao cadastrar livro.",
          variant: "destructive",
        })
      }
    } catch (error) {
      ...
      <header className="bg-gray-800 text-white p-4">
        <div className="flex items-center gap-4">
          ...
          <nav className="flex gap-6">
            ...
            <Link href="/cadastrar-livros" className="text-blue-300">
              Cadastrar livros
            </Link>
          </nav>
        </div>
      ...
      <main className="flex items-center justify-center py-12 px-4">
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            <CardTitle className="text-2xl font-bold">Cadastro de Livros</CardTitle>
          </CardHeader>
          <CardContent>
